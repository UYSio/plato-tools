...
(provide p p->string p-asset-rel-file p-mime-type p-asset-path p-output-landing-pages p-output-entry)

;; TODO check if mime-type can be set with auto-value if #f
(struct p (asset-rel-file mime-type asset-path output-landing-pages output-entry))

(define (p->string params)
  (format
   "asset-rel-file=[~a], mime-type=[~a], asset-path=[~a], output-landing-pages=[~a], output-entry=[~a]"
