...
(provide
 ...
 p-mime-type
 p-asset-path
 p-output-landing-pages
 p-output-entry
 p-the-date
 p-out-file)

;; TODO check if mime-type can be set with auto-value if #f
(struct p (output-root asset-rel-file mime-type asset-path output-landing-pages output-entry the-date out-file))

(define (p->string params)
  (format
   "output-root=[~a] asset-rel-file=[~a], mime-type=[~a], asset-path=[~a], output-landing-pages=[~a], output-entry=[~a] the-date=[~a] out-file=[~a]"
